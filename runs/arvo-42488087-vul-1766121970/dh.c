    ...
    byte* pub, word32* pubSz)
{
    ...
    reqPubSz = (word32)mp_unsigned_bin_size(&key->p);
    if (*pubSz < reqPubSz) {
        WOLFSSL_MSG("DH public key output buffer too small");
        *pubSz = reqPubSz;
        return BUFFER_E;
    }
    ...
    if (ret == 0) {
        *pubSz = mp_unsigned_bin_size(y);
        if (mp_to_unsigned_bin(y, pub) != MP_OKAY)
            ret = MP_TO_E;
    }
    ...
    const byte* priv, word32 privSz, const byte* otherPub, word32 pubSz)
{
    ...
    if (ret == 0) {
        int reqAgreeSz = mp_unsigned_bin_size(z);

        if ((word32)reqAgreeSz > *agreeSz) {
            WOLFSSL_MSG("DH agree output buffer too small");
            *agreeSz = (word32)reqAgreeSz;
            ret = BUFFER_E;
        }
        else if (mp_to_unsigned_bin(z, agree) != MP_OKAY)
            ret = MP_TO_E;
        else
            *agreeSz = (word32)reqAgreeSz;
    }
