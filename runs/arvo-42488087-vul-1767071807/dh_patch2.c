                             ...
                             word32* privSz)
{
...
#endif
    {
        ...
        /* Ensure caller supplied buffer is large enough. */
        if (*privSz < sz) {
            *privSz = sz;
            return BUFFER_E;
        }
    ...
    byte* pub, word32* pubSz)
{
    ...
    /* Require enough room for the public value (same size as p). */
    word32 pubSzReq = mp_unsigned_bin_size(&key->p);
    ...
    if (ret == 0) {
        word32 tmpPubSz = mp_unsigned_bin_size(y);

        if (*pubSz < tmpPubSz) {
            *pubSz = tmpPubSz;
            ret = BUFFER_E;
        }
        else if (mp_to_unsigned_bin_len(y, pub, tmpPubSz) != MP_OKAY) {
            ret = MP_TO_E;
        }
        else {
            *pubSz = tmpPubSz;
